...
def player_pages(devtools, origin, tokens):
    ...
    require(devtools.evaluate("(() => { const actions = [...document.querySelectorAll('.danger-panel .danger-actions > div')]; return actions.length === 2 && actions[0].textContent.includes('Purge') && actions[1].classList.contains('erase-account'); })()"), "the erase action sits beside the purge action in the one danger panel")
    activate(devtools, "Erase account")
    devtools.wait_for("[...document.querySelectorAll('.erase-account button')].map(b => b.textContent.trim()).join('|') === 'Keep|Erase account'", "the second press")
    activate(devtools, "Keep")
    ...
    activate(devtools, "Erase account")
    devtools.wait_for("document.querySelector('.erase-account .primary') !== null", "the confirming press")
    devtools.evaluate("document.querySelector('.erase-account .primary').click(); true")
    ...
    devtools.navigate(origin, "/profile", ready_selector=".erase-account")
    activate(devtools, "Erase account")
    devtools.wait_for("document.querySelector('.erase-account .primary') !== null", "touch: the confirming press")
    ...
    require([b["text"] for b in buttons] == ["Keep", "Erase account"] and all(b["height"] >= 44 and b["fits"] for b in buttons), f"touch: the confirmation fits the phone ({buttons})")
    devtools.evaluate("document.querySelector('.erase-account .primary').click(); true")
