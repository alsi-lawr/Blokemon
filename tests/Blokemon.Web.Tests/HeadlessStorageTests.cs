...
/// <summary>
/// The headless check that a page left while its browser storage is still opening does not block
/// the page that follows it in the same tab. Blokemon.Web on Kestrel with no provider, driven by
/// headless_storage_evidence.py through headless Chrome. Skipped where Chrome or Python is not
/// installed. Run alone with
/// <c>dotnet test --no-build --project tests/Blokemon.Web.Tests -- --treenode-filter "/*/*/HeadlessStorageTests/*"</c>.
/// </summary>
public sealed class HeadlessStorageTests
{
    [Test]
    [Timeout(600_000)]
    public async Task APageLeftDuringItsFirstStorageOpen_DoesNotBlockTheNext(
    ...
    )
    {
        ...
        report.ShouldContain("HEADLESS STORAGE EVIDENCE COMPLETE");
    }
